        ...
        try {
            ...
            if (mes == 0) {
                sql = "select \n"
                        ...
                        + "where \n"
                        + " date(fecha) between '" + desde + "' and '" + hasta + "' and reclutador = " + id + " and sucursal != 'Taller' \n"
                ...
                sql = "select id,contratacion,date(fecha) as fecha,week(fecha) as semana,month(fecha) as mes,year(fecha) as ano\n"
                        + "from arcade_ingresos where year(fecha) = year(curdate()) and reclutador = " + id + " and sucursal != 'Taller' order by ano,fecha;";
            }
            s.rs = s.st.executeQuery(sql);
//            System.out.println(sql);
            List<Solicitud> solicitud = new ArrayList<Solicitud>();
