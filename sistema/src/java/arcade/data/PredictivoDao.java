...
//            System.out.println(sql);
    ...
    //==============================================================================
    public static String select_caja_ciclos(String id_asignaciones) {
        try {
            StartConn s = new StartConn();
            String renglones;
            String sql = "select \n"
            ...
            while (s.rs.next()) {
                Predictivo e = new Predictivo();
                e.setCiclo(s.rs.getString("columna10"));
                predictivo.add(e);
            }
            renglones = "<div class='col s12 m11 cajas' style='max-height:200px; overflow:auto;'>"
                    + "<input type='checkbox' id='check_ciclos_todos'/>"
                    + "<label for='check_ciclos_todos'><b>Ciclos</b></label><br>";

            for (Predictivo c : predictivo) {

                renglones += ""
                        + "<input type='checkbox' id='c_" + c.getCiclo() + "' class='check_ciclos'/>"
                        + "<label for='c_" + c.getCiclo() + "'>" + c.getCiclo() + "</label><br>";
            }
    ...
    //==============================================================================
    public static String select_caja_estados(String id_asignaciones,String cadena_ciclos) {
        try {
            StartConn s = new StartConn();
            String renglones;
            String new_cadena_ciclo = cadena_ciclos.replace("c_", "");
            String sql = "select \n"
                    + "	columna35\n"
                    + "from arcade_basegeneral \n"
                    + "where f_active = 1 and id_asignacion in (" + id_asignaciones + ") and columna10 in (" + new_cadena_ciclo + ") \n"
                    + "group by columna35;";
            s.rs = s.st.executeQuery(sql);
            List<Predictivo> predictivo = new ArrayList<Predictivo>();
            while (s.rs.next()) {
    ...
    //==============================================================================
    public static String select_caja_status(String id_asignaciones, String cadena_ciclos ,String cadena_estados) {
        try {
            StartConn s = new StartConn();
            String renglones;
            String new_cadena_ciclo = cadena_ciclos.replace("c_", "");
            String sql = "select \n"
                    ...
                    + "from arcade_basegeneral \n"
                    + "where f_active = 1 and  id_asignacion in (" + id_asignaciones + ")and columna10 in(" + new_cadena_ciclo + ") and columna35 in(" + cadena_estados + ")\n"
                    + "group by ultimo_estatus_cuenta;";
            s.rs = s.st.executeQuery(sql);
            List<Predictivo> predictivo = new ArrayList<Predictivo>();
            while (s.rs.next()) {
    ...
    //==============================================================================
    public static String select_tabla_predictivo(String id_asignaciones, String cadena_ciclos, String estados, String status, String t_numero, int cr, float resto, String desde) {
        try {
            StartConn s = new StartConn();
            ...
            String sql_tit = "select \n"
                    ...
                    + "    and b.columna10 in (" + new_cadena_ciclo + ")\n"
            ...
            System.out.println(predictivo.size());
            renglones = ""
